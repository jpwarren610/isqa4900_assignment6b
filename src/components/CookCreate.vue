            ...
            <v-card class="login-card">
              ...
              <v-card-text>
                <v-form ref="form" lazy-validation>
                  ...
                  <v-btn v-if="!isUpdate" class="white--text" color="red darken-4" @click="createCook">Save</v-btn>
                  <v-btn v-if="isUpdate" class="white--text" color="red darken-4" @click="updateCook">Update</v-btn>
...
<script>
  ...
  export default {
    ...
    methods: {
      createCook() {
        apiService.addNewCook(this.cook).then(response => {
      ...
      updateCook() {
        apiService.updateCook(this.cook).then(response => {
    ...
    mounted() {
      if (this.$route.params.pk) {
        ...
        this.isUpdate = true;
        apiService.getCook(this.$route.params.pk).then(response => {
          this.cook = response.data;
        }).catch(error => {
          if (error.response.status === 401) {
            router.push("/auth");
